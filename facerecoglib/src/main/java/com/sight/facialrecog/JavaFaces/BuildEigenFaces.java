...
/** BuildEigenFaces.java
 Modified by Sight, April 2014, mpmysight@gmail.com

   Use the training face images in trainingImages\ to create eigen vector and eigenvalue,
   information using PCA. The information is stored in eigen.cache.
   Two sub-directories, eigenfaces\ and reconstructed\, are also generated which
   contain eigen-faces and regenerated images. These are only produced to allow the
   eigen-face process to be checked. 

   Only eigen.cache is used by the recognition process, which is separated out
   into FaceRecognizer.java

*/

import com.sight.facialrecog.util.FileUtils;
...
{  
  ...
  // also save each eigenface (eigenvector) as an image file
  {
    ...
    sortEigenInfo(egVals, egVecs);   // sort Eigenvectors and Eigenvariables
